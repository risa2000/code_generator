...
def generate_func(output_dir="."):
    ...
    functions = [
        CppFunction(name="GetParam", ret_type="int"),
...
def generate_class(output_dir="."):
    ...
    my_class.add_variable(CppVariable(name="m_var1", type="int", value="1"))
    my_class.add_variable(CppVariable(name="m_var2", type="int*"))
    my_class.add_variable(
        CppVariable(
            name="m_var3",
            type="int",
            is_constexpr=True,
    ...
    example_class.add_variable(
        CppVariable(name="m_var1", documentation="/// A number.", type="int"),
    )
    ...
    a2.add_array_item('"Item2"')
    a3 = CppArray(
        name="array3", type="char*", is_static=True, is_const=True, newline_align=True
    )
    a3.add_array_item('"ItemNewline1"')
    a3.add_array_item('"ItemNewline2"')
    ...
    my_class.add_method(
        CppFunction(
            name="VirtualMethod", ret_type="int", is_method=True, is_virtual=True
        )
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate reference code for C++ generator"
    )
    parser.add_argument(
        "-a", "--assets", help="Directory to store generated assets", required=True
    )
